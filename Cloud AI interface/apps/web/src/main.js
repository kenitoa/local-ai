...
const recommendedCombinationSpecs = [
  {
    id: "coding",
    title: "코딩용 최우선 조합",
    badge: "Coding",
    goal: "코드 생성, 리팩터링, 오류 분석에 우선 사용",
    primary: [
      "qwen3-coder:latest",
      "qwen2.5-coder:7b",
      "deepseek-coder:6.7b",
    ...
    support: ["local-assistant", "llama3.1:8b", "llama3.1", "llama3.2:3b", "llama3.2"],
    reason: "코드 전용 마켓 모델이 설치되어 있으면 우선 쓰고, 없으면 현재 작동하는 qwen 계열과 로컬 보조 모델로 코드 검토 경로를 만듭니다.",
    strategy: "coding-review",
  },
  {
    id: "rag",
    title: "문서/RAG 조합",
    badge: "RAG",
    goal: "문서 검색, 요약, 근거 기반 답변",
    primary: ["qwen2.5:7b", "qwen2.5:3b", "qwen2.5", "llama3.2:3b", "llama3.2"],
    support: [
      "bge-m3:latest",
    ...
    reason: "대화 모델과 임베딩 모델을 함께 잡아 검색/RAG 흐름에서 바로 쓸 수 있는 실행 경로를 구성합니다.",
  ...
  },
  {
    ...
    title: "추론/분석 조합",
    badge: "Reasoning",
    goal: "복잡한 분석, 수학, 단계별 판단",
    primary: ["deepseek-r1:7b", "qwen3:4b", "mathstral:7b", "qwen2.5:7b", "qwen2.5"],
    support: ["llama3.1:8b", "llama3.1", "llama3.2:3b", "llama3.2", "local-assistant"],
    reason: "추론 특화 마켓 모델이 있으면 우선 선택하고, 현재 로컬 기본 모델을 보조로 묶어 긴 분석 요청에 맞춥니다.",
    strategy: "reason-then-summarize",
  },
  {
    id: "fast-chat",
    title: "빠른 일반 채팅 조합",
    badge: "Fast",
    goal: "가벼운 질의응답과 빠른 로컬 응답",
    primary: ["tinyllama:1.1b", "llama3.2:1b", "qwen2.5:0.5b", "gemma3:1b", "llama3.2"],
    support: ["local-assistant", "llama3.2:3b", "llama3.2", "llama3.1"],
    reason: "저사양에서 바로 반응하는 모델을 우선하고, 없으면 설치된 기본 채팅 모델로 안정적인 빠른 응답 경로를 만듭니다.",
  ...
  },
  {
    ...
    title: "글쓰기/요약 조합",
    badge: "Writing",
    goal: "문서 초안, 블로그 글, 긴 글 요약",
    primary: ["mistral-nemo:12b", "llama3.1:8b", "llama3.1", "mistral:7b", "gemma3:4b", "llama3.2"],
    support: ["qwen2.5:7b", "qwen2.5:3b", "qwen2.5", "local-assistant"],
    reason: "긴 문맥을 자연스럽게 정리하는 일반 언어 모델을 우선하고, qwen 계열을 보조로 묶어 글 구조와 사실 점검을 보완합니다.",
    strategy: "draft-then-polish",
  },
  {
    id: "korean-multilingual",
    title: "한국어/다국어 조합",
    badge: "Korean",
    goal: "한국어 대화, 번역, 한영 혼합 업무",
    primary: ["qwen2.5:7b", "qwen2.5:3b", "qwen2.5", "qwen3:4b", "mistral-nemo:12b"],
    support: ["llama3.1:8b", "llama3.1", "llama3.2:3b", "llama3.2", "local-assistant"],
    reason: "한국어와 영어가 섞인 요청에 강한 qwen 계열을 우선하고, 범용 채팅 모델을 보조로 둬 응답 안정성을 높입니다.",
    strategy: "multilingual-answer",
  },
  {
    id: "math-data",
    title: "수학/데이터 분석 조합",
    badge: "Math",
    goal: "수식 풀이, 표 해석, 숫자 기반 판단",
    primary: ["mathstral:7b", "deepseek-r1:7b", "qwen3:4b", "qwen2.5:7b", "qwen2.5"],
    support: ["llama3.1:8b", "llama3.1", "local-assistant", "llama3.2"],
    reason: "수학 특화 또는 추론 모델을 우선 선택하고, 일반 채팅 모델을 보조로 둬 계산 과정 설명과 최종 요약을 나눕니다.",
    strategy: "calculate-then-explain",
  },
  {
    id: "project-assistant",
    title: "프로젝트 도우미 조합",
    badge: "Project",
    goal: "로컬 프로젝트 구조 파악, 작업 순서 정리",
    primary: ["local-assistant", "qwen2.5:7b", "qwen2.5:3b", "qwen2.5", "llama3.2"],
    support: ["nomic-embed-text:latest", "nomic-embed-text", "bge-m3:latest", "llama3.1", "llama3.2"],
    reason: "local-ai에 맞춘 보조 모델을 우선하고, 설치된 임베딩 모델이 있으면 프로젝트 검색과 맥락 회수에 같이 쓰도록 묶습니다.",
    strategy: "project-context-helper",
  },
];
...
function getStableOllamaModelNames() {
  ...
  for (const model of state.marketModels.filter((item) => item.installed && item.kind === "ollama")) {
    const modelName = model.modelId || model.name;
    if (modelName && !orderedNames.includes(modelName)) {
      orderedNames.push(modelName);
    }
  }
...
function getAiModelIdentity(model) {
  ...
  const name = normalizeModelLookupName(rawName);
  return `${type}:${name}`;
}
...
function getModelLookupNames(model) {
  return uniqueValues([
    model.key,
    model.id,
    model.name,
...
function getInstalledMarketAiModels() {
  return state.marketModels
    .filter((model) => model.installed && model.kind === "ollama")
    .map((model) => ({
      key: `ollama:${model.modelId || model.name}`,
      ...
      specialty: model.category || "AI market model",
      description: model.description || "AI 마켓에서 설치한 로컬 Ollama 모델입니다.",
      runnable: true,
    }));
}

function getAvailableAiModels() {
  const installedMarketModels = getInstalledMarketAiModels();
  if (state.cloudAiModels.length > 0) {
    return dedupeAiModels([...state.cloudAiModels, ...installedMarketModels]);
  }
  ...
  return dedupeAiModels([...ollamaModels, ...dotnetModels, ...installedMarketModels]);
}
...
function findAvailableModelByPreference(availableModels, preferredNames, predicate = () => true) {
  const normalizedPreferences = preferredNames.map(normalizeModelLookupName);
  return availableModels.find((model) => {
    if (!predicate(model)) {
      return false;
    }

    const names = getModelLookupNames(model);
    return normalizedPreferences.some((preferred) => names.includes(preferred));
  }) ?? null;
}

function appendUniquePart(parts, part) {
  if (part && !parts.some((item) => item.key === part.key)) {
    parts.push(part);
  }
}

function modelComesFromInstalledMarket(part) {
  const names = getModelLookupNames(part);
  return state.marketModels.some((model) =>
    model.installed &&
    model.kind === "ollama" &&
    names.includes(normalizeModelLookupName(model.modelId || model.name)));
}

function getRecommendedCombinationGuides() {
  const availableModels = getAvailableAiModels();
  return recommendedCombinationSpecs
    .map((spec) => {
      const parts = [];
      const primary = findAvailableModelByPreference(
        availableModels,
        spec.primary,
        (model) => model.type === "ollama" && model.runnable !== false,
      );
      const support = findAvailableModelByPreference(availableModels, spec.support);
      const fallback = availableModels.find((model) =>
        model.type === "ollama" &&
        model.runnable !== false &&
        model.key !== primary?.key &&
        model.key !== support?.key);

      appendUniquePart(parts, primary);
      appendUniquePart(parts, support);
      appendUniquePart(parts, fallback);

      if (parts.length === 0) {
        return null;
      }

      const apiModel = (parts.find((part) => part.type === "ollama" && part.runnable !== false) ?? parts[0]).name;
      return {
        ...spec,
        parts: parts.slice(0, 3),
        apiModel,
        sourceLabel: parts.some(modelComesFromInstalledMarket) ? "AI 마켓 설치 모델 포함" : "현재 실행 가능",
      };
    })
    .filter(Boolean);
}

function getRecommendedCombinationId(guide) {
  return `guide-${guide.id}`;
}

function createCombinationFromRecommendedGuide(guide) {
  const parts = guide.parts.map((part) => part.key);
  return normalizeModelCombination({
    id: getRecommendedCombinationId(guide),
    name: guide.title,
    parts,
    expertIds: guide.parts.map(getExpertIdForModelPart).filter(Boolean),
    strategy: guide.strategy,
    apiModel: guide.apiModel,
...
function render() {
  ...
  requestAnimationFrame(() => {
    ...
    if ((state.activePanel === "market" || state.activePanel === "connection") && !state.marketLoaded && !state.busy) {
      state.marketLoaded = true;
...
function renderModelWorkspace() {
  ...
  return `
    <section class="model-workspace" aria-label="AI 모델 선택">
      ...
      ${renderRecommendationGuideSection()}
...
function renderRecommendationGuideSection() {
  const guides = getRecommendedCombinationGuides();
  return `
    <section class="model-panel recommendation-panel" aria-label="추천 조합 가이드">
      <div class="panel-heading">
        <div>
          <span class="section-kicker">Step 3</span>
          <strong>추천 조합 가이드</strong>
          <p>AI 마켓과 로컬 카탈로그에서 현재 실행 가능한 모델만 골라 목적별 최적 실행 경로로 등록합니다.</p>
        </div>
      </div>
      <div class="recommendation-grid">
        ${guides.length > 0
          ? guides.map(renderRecommendationGuideCard).join("")
          : `<p class="empty-text">추천할 수 있는 실행 모델이 아직 없습니다.</p>`}
      </div>
    </section>
  `;
}

function renderRecommendationGuideCard(guide) {
  const selected = state.selectedCombinationId === getRecommendedCombinationId(guide);
  return `
    <button
      class="recommendation-card ${selected ? "selected" : ""}"
      type="button"
      data-recommendation-guide="${escapeHtml(guide.id)}">
      <span class="model-chip">${escapeHtml(guide.badge)}</span>
      <strong>${escapeHtml(guide.title)}</strong>
      <small>${escapeHtml(guide.goal)}</small>
      <p>${escapeHtml(guide.reason)}</p>
      <em>${escapeHtml(guide.parts.map((part) => part.name).join(" + "))}</em>
      <span class="recommendation-route">${escapeHtml(`${guide.sourceLabel} / 실행 모델: ${guide.apiModel}`)}</span>
    </button>
  `;
}
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-recommendation-guide]").forEach((button) => {
    button.addEventListener("click", () => {
      const guide = getRecommendedCombinationGuides().find((item) => item.id === button.dataset.recommendationGuide);
      if (!guide) {
        addLog("추천 조합을 현재 실행 가능한 모델로 만들 수 없습니다.");
        render();
        return;
      }

      const combination = createCombinationFromRecommendedGuide(guide);
      state.modelCombinations = mergeModelCombinations([combination], state.modelCombinations);
      state.selectedCombinationId = combination.id;
      state.model = combination.apiModel || inferCombinationApiModel(combination.parts);
      state.draftCombinationName = "";
      ...
      state.inferenceSummary = `추천 조합 적용: ${combination.name}`;
      writeStoredModelCombinations(state.modelCombinations.filter((item) => !item.serverBacked));
      addLog(`추천 조합 가이드 적용: ${combination.name}`);
      render();
    });
  });
