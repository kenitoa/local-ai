...
$apiSettingsPath = Join-Path $sourceRoot "runtime/semantic-kernel\LocalAI.Api\appsettings.json"
...
if ($apiSettings) {
    ...
    $connectorPath = Join-Path $sourceRoot "runtime\ollama\connector\SemanticKernelOllamaConnector.cs"
...
if (-not $SkipBuild) {
    $solutionPath = Join-Path $sourceRoot "runtime/semantic-kernel\LocalAI.sln"
    try {
